...
import io.turbine.sdk.proto.CommitRequest;
import io.turbine.sdk.proto.HeartbeatRequest;
import io.turbine.sdk.proto.HeartbeatStatus;
import io.turbine.sdk.proto.JoinGroupRequest;
import io.turbine.sdk.proto.LeaveGroupRequest;
import io.turbine.sdk.proto.PartitionAssignment;
import io.turbine.sdk.proto.PartitionCommit;
import io.turbine.sdk.proto.PartitionRead;
import io.turbine.sdk.proto.PartitionResult;
import io.turbine.sdk.proto.ReadRequest;
import io.turbine.sdk.proto.ReadResponse;
import io.turbine.sdk.proto.RejoinRequest;
import io.turbine.sdk.proto.RejoinStatus;
...
public class GroupReader implements AutoCloseable {
    ...
    private GroupReader(ReaderConfig config, TurbineWebSocketClient client, BlockingQueue<byte[]> responseQueue) {
        this.config = config;
        this.client = client;
        this.responseQueue = responseQueue;
    }

    public static GroupReader join(ReaderConfig config) throws TurbineException {
        try {
    ...
    public void startHeartbeat() {
        heartbeatExecutor = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "turbine-heartbeat");
            t.setDaemon(true);
            return t;
        });
    ...
    private void heartbeatTick() {
        ...
        try {
            ...
            io.turbine.sdk.proto.HeartbeatResponse resp = envelope.getHeartbeat();
            if (!resp.getSuccess()) {
    ...
    public void stop() throws TurbineException {
        running.set(false);
    ...
    public List<PartitionResult> poll() throws TurbineException {
        stateLock.readLock().lock();
        State currentState;
        ...
        if (currentState != State.ACTIVE) {
            throw new TurbineException.ProtocolException("Reader not active: " + currentState);
        }
        ...
        try {
            byte[] response = responseQueue.poll(config.getTimeout().toMillis(), TimeUnit.MILLISECONDS);
            if (response == null) {
                throw new TurbineException.TimeoutException("Read timeout");
            }

            ServerMessage envelope = ServerMessage.parseFrom(response);
            if (envelope.getMessageCase() != ServerMessage.MessageCase.READ) {
                throw new TurbineException.ProtocolException("Unexpected response type or empty response");
            }

            ReadResponse resp = envelope.getRead();
            if (!resp.getSuccess()) {
                throw new TurbineException.ProtocolException(
            ...
            throw new TurbineException.ProtocolException("Failed to decode response", e);
        }
    }

    public void commit() throws TurbineException {
        long gen;
        stateLock.readLock().lock();
        try {
            gen = generation;
        ...
        try {
            byte[] response = responseQueue.poll(config.getTimeout().toMillis(), TimeUnit.MILLISECONDS);
            if (response == null) {
                throw new TurbineException.TimeoutException("Commit timeout");
            }
            ...
            io.turbine.sdk.proto.CommitResponse resp = envelope.getCommit();
            if (!resp.getSuccess()) {
                throw new TurbineException.ProtocolException(
    ...
    private void doJoin() throws TurbineException {
        ...
        try {
            byte[] response = responseQueue.poll(config.getTimeout().toMillis(), TimeUnit.MILLISECONDS);
            if (response == null) {
                throw new TurbineException.TimeoutException("Join timeout");
            }
            ...
            io.turbine.sdk.proto.JoinGroupResponse resp = envelope.getJoinGroup();
            if (!resp.getSuccess()) {
                throw new TurbineException.ProtocolException(
    ...
    private void doRejoin(long newGeneration) throws TurbineException {
        stateLock.writeLock().lock();
        try {
        ...
        while (retries < MAX_REJOIN_RETRIES) {
            ...
            try {
                byte[] response = responseQueue.poll(config.getTimeout().toMillis(), TimeUnit.MILLISECONDS);
                if (response == null) {
                    throw new TurbineException.TimeoutException("Rejoin timeout");
                }
                ...
                io.turbine.sdk.proto.RejoinResponse resp = envelope.getRejoin();
                if (!resp.getSuccess()) {
                    throw new TurbineException.ProtocolException(
        ...
        throw new TurbineException.ProtocolException("Rejoin failed after " + MAX_REJOIN_RETRIES + " retries");
    }

    private void doLeave() throws TurbineException {
        try {
            commit();
        } catch (TurbineException e) {
