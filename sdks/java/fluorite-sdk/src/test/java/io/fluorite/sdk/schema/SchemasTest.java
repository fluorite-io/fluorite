...
class SchemasTest {
    ...
    @FluoriteSchema(renames = {@Rename(from = "x", to = "missing")})
    public static class BadRenameTarget {
        public String id;
        public BadRenameTarget() {}
    }

    @FluoriteSchema(deletions = {"id"})
    ...
    void testEvolutionMetadata() {
        ...
        @SuppressWarnings("unchecked")
        Map<String, String> renames = (Map<String, String>) schema.get("fluorite.renames");
        assertEquals("newName", renames.get("old_name"));

        @SuppressWarnings("unchecked")
        List<String> deletions = (List<String>) schema.get("fluorite.deletions");
