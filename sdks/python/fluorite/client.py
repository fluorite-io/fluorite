...
@dataclass
class ClientConfig:
    """Configuration for FluoriteClient."""
...
class FluoriteClient:
    ...
    Usage:
        async with FluoriteClient.connect(ClientConfig(api_key="tb_...")) as client:
    ...
    @classmethod
    async def connect(cls, config: ClientConfig) -> "FluoriteClient":
    ...
    async def send(
        ...
        *,
        ...
        """
        cls = type(obj)
        topic_name = topic or getattr(cls, "__fluorite_topic__", None)
        if topic_name is None:
            raise FluoriteException(
    ...
    async def _resolve_topic(self, name: str) -> int:
        ...
        if name not in self._topic_cache:
            raise FluoriteException(f"Topic not found: {name}")
    ...
    async def __aenter__(self) -> "FluoriteClient":
        return self
