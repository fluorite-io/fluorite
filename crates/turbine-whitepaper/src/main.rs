...
#[derive(Debug, Parser)]
#[command(
    name = "turbine-whitepaper",
    about = "Compile Turbine Typst whitepapers to PDF"
)]
struct Args {
    /// Input Typst document.
    #[arg(
        long,
        default_value = "crates/turbine-whitepaper/typst/eventbus_whitepaper.typ"
    )]
    input: PathBuf,
    /// Output PDF path.
    #[arg(
        long,
        default_value = "crates/turbine-whitepaper/out/eventbus_whitepaper.pdf"
    )]
    output: PathBuf,
