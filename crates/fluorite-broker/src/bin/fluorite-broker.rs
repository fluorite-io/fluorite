...
fn init_tracing() -> Result<Option<SdkTracerProvider>> {
    let env_filter = EnvFilter::from_default_env().add_directive("fluorite_broker=info".parse()?);
    ...
    let service_name = env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "fluorite-broker".into());
    ...
    let tracer = provider.tracer("fluorite-broker");
...
async fn main() -> Result<()> {
    ...
    let data_dir = env::var("DATA_DIR").unwrap_or_else(|_| "/tmp/fluorite".to_string());

    info!("Starting fluorite-broker");
