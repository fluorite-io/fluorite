...
static WRITER_STATE_ROWS_BUMP: LazyLock<StdMutex<Bump>> = LazyLock::new(|| {
    let capacity_bytes = std::env::var("FLUORITE_WRITER_STATE_BUMP_BYTES")
        .ok()
...
pub struct BrokerConfig {
    ...
    #[cfg(feature = "iceberg")]
    pub iceberg: Option<fluorite_iceberg::IcebergConfig>,
}
    ...
    pub async fn with_coordinator_config(
        ...
        let iceberg_buffer = {
            if let Some(ref iceberg_config) = config.iceberg {
                match fluorite_iceberg::iceberg_writer::TableWriter::new(
                ...
                .await
                {
                    Ok(writer) => {
                        ...
                        Some(fluorite_iceberg::IcebergBuffer::new(
                            iceberg_config.clone(),
                            writer,
                        ))
                    }
...
async fn handle_message<S: ObjectStore + Send + Sync + 'static>(
    ...
    let (msg, used) = match decode_client_message(data) {
        ...
        Err(_) => {
            ...
            if let Ok((key, _)) = fluorite_wire::varint::decode_u64(data) {
                let field = key >> 3;
...
mod tests {
    ...
    fn test_encode_error_response_shapes() {
        ...
        let (produce_msg, produce_len) = fluorite_wire::decode_server_message(&append).unwrap();
        ...
        let (fetch_msg, fetch_len) = fluorite_wire::decode_server_message(&read).unwrap();
        ...
        let (join_msg, join_len) = fluorite_wire::decode_server_message(&join).unwrap();
        ...
        let (commit_msg, commit_len) = fluorite_wire::decode_server_message(&commit).unwrap();
