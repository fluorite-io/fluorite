...
//!
//! Consumes explicit SDK metadata (`flourine.renames`, `flourine.deletions`)
//! from schema JSON rather than inferring renames from Avro aliases.
//!
//! Operations:
//! - `flourine.renames`: `{"old_name": "new_name"}` → `RenameColumn`
//! - `flourine.deletions`: `["removed_field"]` → `DeleteColumn`
...
///
/// Reads `flourine.renames` and `flourine.deletions` from the new schema's
...
pub fn compute_evolution(
    ...
    // 1. Renames from flourine.renames: {"old_name": "new_name"}
    let mut renames_reverse: HashMap<&str, &str> = HashMap::new();
    if let Some(renames) = new_json.get("flourine.renames").and_then(|v| v.as_object()) {
        for (old_name, new_name) in renames {
            let new_name_str = new_name.as_str()
                .ok_or_else(|| IcebergError::Schema(
                    format!("flourine.renames value for '{}' must be a string", old_name),
                ))?;
    ...
    // 2. Deletions from flourine.deletions: ["removed_field"]
    if let Some(deletions) = new_json.get("flourine.deletions").and_then(|v| v.as_array()) {
        for name in deletions {
            let name_str = name.as_str()
                .ok_or_else(|| IcebergError::Schema(
                    "flourine.deletions entries must be strings".into(),
                ))?;
...
mod tests {
    ...
    fn test_rename_via_metadata() {
        ...
        let new = json!({
            ...
            "flourine.renames": {"old_name": "new_name"}
        });
    ...
    fn test_remove_field_no_op_without_metadata() {
        ...
        // Without flourine.deletions, removing a field produces no ops
        let evo = compute_evolution(&new, &old).unwrap();
        assert!(evo.is_empty());
    }
    ...
    fn test_combined_evolution() {
        ...
        let new = json!({
            ...
            "flourine.renames": {"old_id": "new_id"},
            "flourine.deletions": ["removed"]
        });
