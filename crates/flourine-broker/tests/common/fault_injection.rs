...
impl flourine_broker::ObjectStore for FaultyObjectStore {
    ...
    async fn size(
        &self,
        key: &str,
    ) -> Result<u64, flourine_broker::object_store::ObjectStoreError> {
...
impl CrashableWsBroker {
    ...
    async fn boot_with_config(&mut self, buffer: BufferConfig) {
        ...
        let handle = tokio::spawn(async move {
            let _ = flourine_broker::run(state).await;
        });
    ...
    async fn boot_with_shutdown(&mut self) {
        ...
        let handle = tokio::spawn(async move {
            ...
            let _ = flourine_broker::run_with_shutdown(state, shutdown).await;
        });
        tokio::time::sleep(Duration::from_millis(200)).await;
        self.addr = addr;
        self.server_handle = Some(handle);
