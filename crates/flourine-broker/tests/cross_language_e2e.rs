...
//! - Java 17+ with the SDK built: cd sdks/java/flourine-sdk && mvn package
...
async fn run_java_writer(
    ...
    let dependency_glob = root.join("sdks/java/flourine-sdk/target/dependency/*");
...
fn java_sdk_available() -> bool {
    let root = project_root();
    let jar_path = root.join("sdks/java/flourine-sdk/target/flourine-sdk-0.1.0.jar");
    let dep_dir = root.join("sdks/java/flourine-sdk/target/dependency");
    let class_dir = root.join("sdks/java/flourine-sdk/target/classes/io/flourine/sdk");
...
fn ensure_java_sdk_built() -> Result<(), String> {
    ...
    let root = project_root();
    let sdk_dir = root.join("sdks/java/flourine-sdk");
