//! End-to-end load tests against a real flourine-broker instance.
//!
...
fn ensure_metrics_registered() {
    let _ = METRICS_REGISTERED.get_or_init(|| {
        flourine_broker::metrics::register_metrics();
    });
}
...
fn hot_histogram_snapshot() -> HashMap<String, HistogramSample> {
    ...
    for family in flourine_broker::metrics::REGISTRY.gather() {
        let name = family.get_name();
        if !name.starts_with("flourine_ws_") && !EXTRA_HISTOGRAMS.contains(&name) {
            continue;
        }
...
fn encode_client_frame(msg: ClientMessage, capacity: usize) -> Vec<u8> {
    ...
    loop {
        ...
        match encode_client_message(&msg, &mut buf) {
            ...
            Err(flourine_wire::EncodeError::BufferTooSmall { needed, .. }) => {
...
fn init_otel_capture(sample_ratio: f64) -> &'static OTelCapture {
    OTEL_CAPTURE.get_or_init(|| {
        ...
        let provider = SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("flourine-broker-e2e-load");

        // If another test already initialized a subscriber, continue without failing.
        let _ = tracing_subscriber::registry()
            .with(EnvFilter::new("off,flourine_broker=debug"))
...
async fn run_load_scenario(scenario: LoadScenario, capture_otel: bool) -> LoadRunResult {
    ...
    let otel = if capture_otel {
        let sample_ratio = env_f64("FLOURINE_OTEL_SAMPLE_RATIO", 0.001);
...
async fn test_e2e_load_smoke() {
    ...
    if let Ok(min_rps) = std::env::var("FLOURINE_LOAD_MIN_RPS") {
        let min_rps: f64 = min_rps.parse().expect("invalid FLOURINE_LOAD_MIN_RPS");
        assert!(
...
async fn test_e2e_load_one_million_requests() {
    ...
    let fetch_timeout = Duration::from_secs(env_u64("FLOURINE_LOAD_FETCH_TIMEOUT_SECS", 900));
    let enable_otel = std::env::var("FLOURINE_LOAD_ENABLE_OTEL")
