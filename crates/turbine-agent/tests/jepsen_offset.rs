...
use turbine_agent::{LocalFsStore, ObjectStore, TbinReader};
use turbine_common::ids::{Offset, PartitionId, TopicId};
use turbine_common::types::Record;

use common::{AgentConfig, AgentState, TestDb, produce_records};
...
async fn test_watermark_never_regresses() {
    ...
    let state = Arc::new(AgentState::new(db.pool.clone(), store, config));
