...
//! - Java 17+ with the SDK built: cd sdks/java/turbine-sdk && mvn package
...
async fn run_java_writer(
    ...
    let root = project_root();
    let jar_path = root.join("sdks/java/turbine-sdk/target/turbine-sdk-0.1.0.jar");
    let dependency_glob = root.join("sdks/java/turbine-sdk/target/dependency/*");
    ...
    let output = TokioCommand::new("java")
        .arg("-cp")
        .arg(&classpath)
        .arg("io.turbine.test.JavaWriter")
        .arg(url)
        .arg(topic_id.to_string())
        .arg(partition_id.to_string())
        .arg(num_records.to_string())
...
async fn run_java_reader(
    ...
    let output = TokioCommand::new("java")
        .arg("-cp")
        .arg(&classpath)
        .arg("io.turbine.test.JavaReader")
        .arg(url)
        .arg(topic_id.to_string())
        .arg(partition_id.to_string())
        .arg(expected_count.to_string())
...
fn java_sdk_available() -> bool {
    let root = project_root();
    let jar_path = root.join("sdks/java/turbine-sdk/target/turbine-sdk-0.1.0.jar");
    let dep_dir = root.join("sdks/java/turbine-sdk/target/dependency");
    let class_dir = root.join("sdks/java/turbine-sdk/target/classes/io/turbine/sdk");
...
fn ensure_java_sdk_built() -> Result<(), String> {
    ...
    let root = project_root();
    let sdk_dir = root.join("sdks/java/turbine-sdk");
