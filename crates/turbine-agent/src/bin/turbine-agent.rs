...
fn init_tracing() -> Result<Option<SdkTracerProvider>> {
    let env_filter = EnvFilter::from_default_env().add_directive("turbine_agent=info".parse()?);
    ...
    let service_name = env::var("OTEL_SERVICE_NAME").unwrap_or_else(|_| "turbine-broker".into());
    ...
    let tracer = provider.tracer("turbine-broker");
...
async fn main() -> Result<()> {
    ...
    let data_dir = env::var("DATA_DIR").unwrap_or_else(|_| "/tmp/turbine".to_string());

    info!("Starting turbine-broker");
    ...
    // Create broker state
    let agent_state = BrokerState::with_coordinator_config(
        pool.clone(),
        store,
        agent_config,
    ...
    let ws_server = run_with_shutdown(agent_state, ws_shutdown);
