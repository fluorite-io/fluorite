...
pub struct CoordinatorConfig {
    ...
    /// Session timeout for detecting dead consumers.
...
impl Coordinator {
    ...
    pub async fn join_group(
        ...
        // 1. Initialize group if first consumer
        self.initialize_group(&mut tx, group_id, topic_id).await?;

        // 2. Register member (upsert)
        sqlx::query(
            r#"
            INSERT INTO consumer_members (group_id, topic_id, consumer_id, last_heartbeat)
            VALUES ($1, $2, $3, NOW())
            ON CONFLICT (group_id, topic_id, consumer_id)
        ...
        // 4. Get live members
        let session_timeout_secs = self.config.session_timeout.as_secs() as i32;
        let members: Vec<String> = sqlx::query_scalar(
            r#"
            SELECT consumer_id FROM consumer_members
            WHERE group_id = $1 AND topic_id = $2
        ...
        let partition_count: i32 = sqlx::query_scalar(
            "SELECT partition_count FROM topics WHERE topic_id = $1",
        )
        .bind(topic_id.0 as i32)
        .fetch_one(&mut *tx)
        .await?;

        // 6. Compute assignment for this consumer
        let assigned = compute_assignment(consumer_id, &members, partition_count as u32);
        ...
        for p in assigned {
            let result: Option<(i32, i64)> = sqlx::query_as(
                ...
                WHERE group_id = $4 AND topic_id = $5 AND partition_id = $6
                  AND (consumer_id IS NULL OR lease_expiry < NOW())
    ...
    pub async fn heartbeat(
        ...
        let updated = sqlx::query(
            r#"
            UPDATE consumer_members
            SET last_heartbeat = NOW()
            WHERE group_id = $1 AND topic_id = $2 AND consumer_id = $3
            "#,
        )
        .bind(group_id)
        .bind(topic_id.0 as i32)
        ...
        sqlx::query(
            ...
            WHERE group_id = $2 AND topic_id = $3 AND consumer_id = $4
            "#,
        )
        .bind(lease_secs)
        .bind(group_id)
        ...
        if !expired.is_empty() {
            ...
            sqlx::query(
                r#"
                DELETE FROM consumer_members
                WHERE group_id = $1 AND topic_id = $2
                AND consumer_id = ANY($3)
                "#,
            )
            .bind(group_id)
            .bind(topic_id.0 as i32)
            ...
            sqlx::query(
                r#"
                UPDATE consumer_assignments
                SET consumer_id = NULL, lease_expiry = NULL
                WHERE group_id = $1 AND topic_id = $2
                AND consumer_id = ANY($3)
                "#,
            )
            ...
            current_gen = sqlx::query_scalar(
                "SELECT generation FROM consumer_groups WHERE group_id = $1 AND topic_id = $2",
            )
            .bind(group_id)
            .bind(topic_id.0 as i32)
            .fetch_one(&mut *tx)
        ...
        let status = if current_gen > consumer_generation.0 as i64 {
    ...
    pub async fn rejoin(
        ...
        // 4. Compute what this consumer should own
        let assigned: std::collections::HashSet<u32> =
            compute_assignment(consumer_id, &members, partition_count as u32)
                .into_iter()
                .collect();

        // 5. Get what this consumer currently owns
        let currently_owned: Vec<i32> = sqlx::query_scalar(
            r#"
            SELECT partition_id FROM consumer_assignments
            WHERE group_id = $1 AND topic_id = $2 AND consumer_id = $3
            "#,
        )
        .bind(group_id)
        ...
        if !excess.is_empty() {
            sqlx::query(
                ...
                SET consumer_id = NULL, lease_expiry = NULL
                WHERE group_id = $1 AND topic_id = $2
                AND partition_id = ANY($3) AND consumer_id = $4
                "#,
            )
            .bind(group_id)
            .bind(topic_id.0 as i32)
        ...
        if !kept.is_empty() {
            sqlx::query(
                r#"
                UPDATE consumer_assignments
                SET lease_expiry = NOW() + make_interval(secs => $1), generation = $2
                WHERE group_id = $3 AND topic_id = $4
                AND partition_id = ANY($5) AND consumer_id = $6
                "#,
            )
            .bind(lease_secs)
        ...
        if !kept.is_empty() {
            let kept_offsets: Vec<(i32, i64)> = sqlx::query_as(
                r#"
                SELECT partition_id, committed_offset FROM consumer_assignments
    ...
    pub async fn leave_group(
        ...
        // 1. Release this consumer's partitions
        sqlx::query(
            r#"
            UPDATE consumer_assignments
            SET consumer_id = NULL, lease_expiry = NULL
    ...
    pub async fn commit_offset(
        ...
        let result = sqlx::query(
            ...
            WHERE group_id = $2 AND topic_id = $3 AND partition_id = $4
              AND consumer_id = $5
            "#,
        )
    ...
    /// Initialize a consumer group and its partition assignments.
    async fn initialize_group(
        ...
        for p in 0..partition_count {
            sqlx::query(
                r#"
                INSERT INTO consumer_assignments
                (group_id, topic_id, partition_id, committed_offset)
                VALUES ($1, $2, $3, 0)
                ON CONFLICT DO NOTHING
                "#,
            )
...
/// Compute partition assignment for a consumer.
///
/// Uses deterministic range-based assignment. Given a sorted member list
/// and partition count, any agent computes the same result.
pub fn compute_assignment(consumer_id: &str, members: &[String], partition_count: u32) -> Vec<u32> {
    ...
    let my_index = match sorted_members.iter().position(|m| m == consumer_id) {
        Some(idx) => idx,
        None => return vec![],
    };
...
mod tests {
    ...
    fn test_compute_assignment_single_consumer() {
        let members = vec!["consumer-a".to_string()];
        let result = compute_assignment("consumer-a", &members, 10);
    ...
    fn test_compute_assignment_three_consumers_uneven() {
        ...
        // 10 partitions / 3 consumers = 3 each + 1 extra for first consumer
        let result_a = compute_assignment("a", &members, 10);
    ...
    fn test_compute_assignment_more_consumers_than_partitions() {
        ...
        // 3 partitions, 5 consumers - some get nothing
    ...
    fn test_compute_assignment_order_independent() {
        // Members in different order should produce same assignment
