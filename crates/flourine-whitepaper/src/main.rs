...
#[derive(Debug, Parser)]
#[command(
    name = "flourine-whitepaper",
    about = "Compile Flourine Typst whitepapers to PDF"
)]
struct Args {
    /// Input Typst document.
    #[arg(
        long,
        default_value = "crates/flourine-whitepaper/typst/eventbus_whitepaper.typ"
    )]
    input: PathBuf,
    /// Output PDF path.
    #[arg(
        long,
        default_value = "crates/flourine-whitepaper/out/eventbus_whitepaper.pdf"
    )]
    output: PathBuf,
