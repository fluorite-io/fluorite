...
//! let serializer_config = &mut turbine::avro::ser::SerializerConfig::new(&schema);
//! let mut serializer_state =
//! 	turbine::avro::ser::SerializerState::from_writer(Vec::new(), serializer_config);
//!
...
//! let mut serializer_state = turbine::avro::ser::SerializerState::from_writer(
//! 	{
...
use super::schema::{self_referential::*, UnionVariantLookupKey};
...
/// ```
/// # use turbine::avro::{ser, Schema};
/// let schema: Schema = r#""int""#.parse().unwrap();
/// let serializer_config = &mut ser::SerializerConfig::new(&schema);
///
/// let mut serialized: Vec<u8> = turbine::avro::to_datum_vec(&3, serializer_config).unwrap();
/// assert_eq!(serialized, &[6]);
///
/// // reuse config & output buffer across serializations for ideal performance (~40% perf gain)
/// serialized.clear();
/// let serialized = turbine::avro::to_datum(&4, serialized, serializer_config).unwrap();
